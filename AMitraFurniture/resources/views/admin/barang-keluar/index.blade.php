    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white d-flex justify-content-between align-items-center">
            <h6 class="fw-bold mb-0">
                <i class="fas fa-table me-2"></i>Riwayat Barang Keluar
            </h6>
                        ...
                        @forelse($barangKeluar as $item)
                            <tr>
                                <td class="text-nowrap">
                                    <div class="fw-semibold">{{ optional($item->created_at)->format('d M Y') }}</div>
                                ...
                                <td class="text-center">
                                    <span class="badge bg-danger fs-6">-{{ number_format($item->quantity) }}</span>
                                </td>
                                <td>{{ $item->reference ?: '-' }}</td>
                                <td>
                                    <span class="text-truncate d-block small" style="max-width: 200px;" title="{{ $item->notes }}">
                                        {{ $item->notes ?: '-' }}
                                    </span>
                                </td>
                            </tr>
            ...
            <div class="card-footer bg-white py-3">
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                    <div class="text-muted small">
                        Menampilkan <strong>{{ $barangKeluar->firstItem() }}</strong> -
                        <strong>{{ $barangKeluar->lastItem() }}</strong> dari
                        <strong>{{ $barangKeluar->total() }}</strong> transaksi
                    </div>
