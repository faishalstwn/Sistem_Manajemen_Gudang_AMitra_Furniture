...
const props = defineProps({ location: Object, zonaList: Array })

const form = useForm({
    _method: 'PUT',
    kode: props.location.kode,
    zona: props.location.zona,
    baris: props.location.baris,
    kolom: props.location.kolom,
    kapasitas: props.location.kapasitas,
    deskripsi: props.location.deskripsi || '',
})

function submit() {
    form.post(`/admin/lokasi-gudang/${props.location.id}`)
}
</script>

<template>
    <Head :title="`Edit Lokasi ${location.kode}`" />
    <div class="max-w-2xl mx-auto">
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <p class="text-gray-500 mt-1">Perbarui data lokasi {{ location.kode }}.</p>
            </div>
        </div>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input v-model="form.zona" type="text" required
                        ...
                        <datalist id="zona-list"><option v-for="z in zonaList" :key="z" :value="z" /></datalist>
