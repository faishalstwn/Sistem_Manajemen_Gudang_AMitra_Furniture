...
function submit() {
    form.post(`/admin/gudang/${props.product.id}/adjustment`)
}
</script>

<template>
    <Head :title="`Penyesuaian — ${product.name}`" />
    <div class="max-w-2xl mx-auto">
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <p class="text-gray-500 mt-1">Koreksi stok fisik untuk <span class="font-semibold text-gray-700">{{ product.name }}</span></p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
            <div class="flex items-center gap-4 p-4 bg-blue-50 rounded-xl">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500">Stok sistem saat ini: <span class="font-bold text-[#1565C0]">{{ product.stock }}</span></p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
            <form @submit.prevent="submit" class="space-y-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Stok Fisik Sebenarnya *</label>
                    ...
                    <p v-if="form.quantity && form.quantity != product.stock" class="mt-1.5 text-xs font-medium"
                       :class="Number(form.quantity) > product.stock ? 'text-emerald-600' : 'text-red-600'">
                        Selisih: {{ Number(form.quantity) > product.stock ? '+' : '' }}{{ Number(form.quantity) - product.stock }} unit
                    </p>
                    <p v-if="form.errors.quantity" class="mt-1 text-xs text-red-500">{{ form.errors.quantity }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Alasan Penyesuaian *</label>
                    <textarea v-model="form.keterangan" rows="3" required
                              class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-[#1565C0]/40 focus:border-[#1565C0] focus:bg-white transition-all"
                              placeholder="Contoh: Hasil stock opname fisik 10 Juni 2026"></textarea>
