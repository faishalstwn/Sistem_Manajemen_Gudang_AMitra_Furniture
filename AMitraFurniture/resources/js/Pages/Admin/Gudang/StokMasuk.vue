...
function submit() {
    form.post(`/admin/gudang/${props.product.id}/stok-masuk`)
}
</script>

<template>
    <Head :title="`Stok Masuk — ${product.name}`" />
    <div class="max-w-2xl mx-auto">
        <div class="flex items-center gap-4 mb-8">
            <Link href="/admin/gudang/kelola" class="w-10 h-10 bg-white border border-gray-200 rounded-xl flex items-center justify-center text-gray-600 hover:bg-gray-50 hover:text-emerald-600 transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            </Link>
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Stok Masuk</h1>
                <p class="text-gray-500 mt-1">Tambahkan stok untuk <span class="font-semibold text-gray-700">{{ product.name }}</span></p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
            <div class="flex items-center gap-4 p-4 bg-emerald-50 rounded-xl">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500">Stok saat ini: <span class="font-bold text-emerald-600">{{ product.stock }}</span></p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
            <form @submit.prevent="submit" class="space-y-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Jumlah Stok Masuk *</label>
                ...
                <div>
                    ...
                    <textarea v-model="form.keterangan" rows="3"
                              class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500/40 focus:border-emerald-400 focus:bg-white transition-all"
                              placeholder="Catatan / alasan penambahan stok"></textarea>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit" :disabled="form.processing"
                        ...
                        {{ form.processing ? 'Menyimpan...' : 'Simpan Stok Masuk' }}
                    </button>
                </div>
            </form>
        </div>
